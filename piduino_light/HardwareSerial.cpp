...
//
// Modified for use on Raspberry PI with the MOVI(TM) Arduino 
...
void HardwareSerial::setDevice(const char* deviceName)
{
    // Override device name from environment
    char* e = getenv("MOVI_SERIAL");
    if (e)
	_deviceName = e;
     else 
...
const char *HardwareSerial::getDevice()
{
    	return _deviceName;
}

bool HardwareSerial::begin(int baud)
{
    if (openDevice())
	return setBaud(baud);
    else return false;
...
{
    fprintf(stderr, "HardwareSerial::peek not implemented\n");
    return 0;
}
...
bool HardwareSerial::setBaud(int baud)
{
    ...
    if (!_istty) { // be Arduino compatible
